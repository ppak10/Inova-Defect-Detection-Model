...
class SegHead(nn.Module):
    """Patch grid (+ part mask) -> dense class logits."""
    ...
    def __init__(self, embed_dim: int, channels: int, n_classes: int, out_size: int):
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(channels // 2, n_classes, 1),
        )

    def forward(self, grid: torch.Tensor, part_mask: torch.Tensor) -> torch.Tensor:
        m = F.interpolate(part_mask[:, None].float(), size=grid.shape[-2:], mode="nearest")
        logits = self.net(torch.cat([grid, m], dim=1))
...
class DefectSegmenter(nn.Module):
    ...
    def forward(
        ...
        return self.head(grid, part_mask)  # (B, C, logits_size, logits_size)
