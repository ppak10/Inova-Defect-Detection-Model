"""
Entry point: uv run python -m runs.v01.train

Trains DefectSegmenter on the Peregrine cache (see prepare.py), holding
out one build for cross-build validation. Checkpoints land in
runs/v01/checkpoints/ (gitignored); wandb project inova-defect-v01.

Smoke test:
    uv run python -m runs.v01.train --max-cases 8 --epochs 1 --no-wandb
"""
...
def main() -> None:
    ...
    p.add_argument("--region-loss-weight", type=float, default=1.0)
    p.add_argument("--workers", type=int, default=8)
    p.add_argument("--max-cases", type=int, default=None, help="smoke: cap layers per split")
    p.add_argument("--no-wandb", action="store_true")
    p.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    args = p.parse_args()

    train_ds = PeregrineSequenceDataset(
        builds=TRAIN_BUILDS, k_frames=args.k_frames, logits_size=args.logits_size
    )
    val_ds = PeregrineSequenceDataset(
        builds=VAL_BUILD, k_frames=args.k_frames, logits_size=args.logits_size, augment=False
    )
    if args.max_cases:
    ...
    if not args.no_wandb:
        ...
        run = wandb.init(project="inova-defect-v01", config=vars(args))
    ...
    for epoch in range(args.epochs):
        ...
        line = f"epoch {epoch}: " + " ".join(
            f"{k.split('/')[-1]}={v:.4f}" for k, v in {**train_stats, **metrics}.items()
        )
        print(line)
        torch.save(
        ...
        if mean_ap == mean_ap and mean_ap > best:  # nan-safe
            best = mean_ap
            torch.save(
                {"model": model.state_dict(), "config": vars(args), "epoch": epoch},
