"""
Entry point: uv run python -m runs.v04.evaluate --checkpoint <path>
...
Smoke test:
    uv run python -m runs.v04.evaluate --checkpoint runs/v04/checkpoints/best.pt --max-cases 8
"""
...
def evaluate(model, dl, device: str) -> dict[str, float]:
    ...
    for batch in dl:
        ...
        hscores.append(region_logits.float()[:, :, ANOMALY_IDX][rv.to(device)].sigmoid().cpu().numpy())
        labels.append(batch["labels"][rv].numpy())
    scores, hscores, labels = np.concatenate(scores), np.concatenate(hscores), np.concatenate(labels)
    out = {}
    for j, c in enumerate(PEREGRINE_CLASSES):
        pos = labels[:, j] > LABEL_FRACTION_THRESHOLD
        out[c] = average_precision(scores[:, j], pos)
        out[f"{c}/head"] = average_precision(hscores[:, j], pos)
...
def main() -> None:
    ...
    for c in PEREGRINE_CLASSES:
        ap, aph = results[c], results[f"{c}/head"]
        fmt = lambda v: "nan" if np.isnan(v) else f"{v:.4f}"
        print(f"  {c:22s} dense {fmt(ap)}  head {fmt(aph)}  (n_pos {results[f'{c}/n_pos']})")
