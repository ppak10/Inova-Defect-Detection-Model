"""
Entry point: uv run python -m runs.v03.train

v03: build-3 holdout (debris 649 / short-feed 378 / swelling 1,929
test positives — the classes that matter for the Inova), region loss
weight 3.0 (probe showed region-pooled frozen features carry the
part-state signal). Otherwise inherits v02 mechanics. wandb project
inova-defect-v03; checkpoints in runs/v03/checkpoints/.

Smoke test:
    uv run python -m runs.v03.train --max-cases 8 --epochs 1 --no-wandb
"""
...
def main() -> None:
    ...
    if not args.no_wandb:
        ...
        run = wandb.init(project="inova-defect-v03", config=vars(args))
