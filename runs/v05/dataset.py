"""
Datasets for v05 = v04 with the paste and photometric fixes:
- Debris paste v2: feathered-alpha blending with gain/scale jitter (no
  hard copy edges to shortcut on), pasted anywhere on the bed, and —
  the v04 bug fix — REGION labels updated for pasted pixels so the
  region head finally receives paste supervision.
- Photometric stack 4 -> 7 channels: adds local-statistics maps (box
  std / box |diff|) so the head is handed the short-feed probe's
  winning features instead of having to learn to compute them.
...
class PeregrineSequenceDataset(Dataset):
    ...
    def _paste_debris(self, images, class_bits, rng) -> np.ndarray:
        """In-place paste of 1-3 debris patches into both current-layer
        channels + the debris class bit (never the prev frame).
        Feathered alpha, gain/scale jitter. Returns the pasted-pixel
        mask (full res) so region labels can be updated."""
        import cv2

        pasted = np.zeros(images.shape[1:], dtype=bool)
        for _ in range(int(rng.integers(1, 4))):
            ...
            hw = (max(int(pm.shape[1] * scale), 4), max(int(pm.shape[0] * scale), 4))
            patch = np.stack([cv2.resize(patch[c], hw) for c in range(2)])
            pm = cv2.resize(pm.astype(np.float32), hw) > 0.5
            h, w = pm.shape
            if h >= images.shape[1] or w >= images.shape[2]:
                continue
            alpha = cv2.GaussianBlur(pm.astype(np.float32), (0, 0), 1.5) * pm
            ...
            for ch in range(2):
                ...
                images[ch, y : y + h, x : x + w] = tgt * (1 - alpha) + patch[ch] * gain * alpha
            cb = class_bits[y : y + h, x : x + w]
            cb[pm] |= np.uint16(1 << self.DEBRIS_BIT)
            pasted[y : y + h, x : x + w] |= pm
        return pasted
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        if self.augment and self.debris_bank and rng.random() < 0.5:
            ...
            pasted_mask = self._paste_debris(images, class_bits, rng)
        ...
        if pasted_mask is not None and pasted_mask.any():
            # v04 bug fix: pasted debris must reach the REGION labels too.
            # Derive debris pixels from class_bits (flip-safe — the flips
            # block flips class_bits and region_map together, while the
            # returned pasted_mask is pre-flip). Includes natural debris;
            # max() below keeps labels consistent either way.
            debris_col = PEREGRINE_CLASSES.index("debris")
            pasted_now = ((class_bits >> PEREGRINE_ALL_CLASSES.index("debris")) & 1).astype(bool)
            under = region_map[pasted_now]
            under = under[under >= 0]
            if len(under):
                counts = np.bincount(under, minlength=n_regions)
                areas = np.bincount(region_map[region_map >= 0], minlength=n_regions)
                frac = counts / np.maximum(areas, 1)
                labels[:, debris_col] = np.maximum(labels[:, debris_col], frac[:n_regions])
        ...
        def _box_std(a, k=15):
            m = _cv2.blur(a, (k, k))
            return np.sqrt(np.clip(_cv2.blur(a * a, (k, k)) - m * m, 0, None))

        stack = np.stack(
            [
                pr, ps, ps - pp, pr - pp,
                _box_std(pr),
                _cv2.blur(np.abs(pr - pp), (15, 15)),
                _cv2.blur(np.abs(ps - pp), (15, 15)),
            ]
        )
        stack = F.interpolate(
