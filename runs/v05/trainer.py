"""
Training loop for v05 = v03 + region-HEAD loss + photometric input.
...
class Trainer:
    ...
    def validate(self, epoch: int) -> dict:
        ...
        for j, c in enumerate(PEREGRINE_CLASSES):
            ...
            if not np.isnan(aph):
                aps.append(aph)
        # v05 checkpoint metric: per class the BEST of the two ranking
        # paths, so neither head is sacrificed for the other
        best_per_class = [
            np.nanmax([metrics[f"val/ap_{c}"], metrics[f"val/aph_{c}"]])
            for c in PEREGRINE_CLASSES
        ]
        metrics["val/mean_ap"] = float(np.nanmean(best_per_class))
        metrics["val/mean_ap_head"] = float(np.mean(aps)) if aps else float("nan")
