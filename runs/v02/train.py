"""
Entry point: uv run python -m runs.v02.train

Trains on the shared runs/v01 Peregrine cache (all 17.5k layers) with
v02 changes: prev_scan temporal frame, cosine LR, per-class region
loss weighting, 518-px dataloader frames. wandb project
inova-defect-v02; checkpoints in runs/v02/checkpoints/.

Smoke test:
    uv run python -m runs.v02.train --max-cases 8 --epochs 1 --no-wandb
"""
...
def main() -> None:
    ...
    if not args.no_wandb:
        ...
        run = wandb.init(project="inova-defect-v02", config=vars(args))
